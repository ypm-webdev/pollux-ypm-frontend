...
const ImageryMultiContainer: React.FC<Params> = ({
  manifestIiif,
  manifest2dRti,
  manifest2dZst,
  manifest2d360,
  ...
  const getFirstActiveTab = (): string => {
  ...
  if (soloKey) {
    if (soloKey === '2d')
      return <Clover manifest={manifestIiif} solo={isSolo['2d']} />
    if (soloKey === '2drti') return <h2>OpenLime - TODO</h2>
    if (soloKey === '2dzst')
      return <CIZStack manifest={manifest2dZst} solo={isSolo['2dzst']} />
    if (soloKey === '2d360')
      return <CI360 manifest={manifest2d360} solo={isSolo['2d360']} />
    if (soloKey === '3dobj')
      return <Three manifest={manifest3dObj} solo={isSolo['3dobj']} />
    if (soloKey === '3dvol')
      return <div>3D Volume Manifest: {manifest3dVol}</div>
  }
